...
{
    public partial class DossierConsultationGynecoPaiementFrm : Form
    {
        public DossierConsultationGynecoPaiementFrm()
        {
            InitializeComponent();
        }
        ...
        clsdossierconsultationgynecologique consultation = new clsdossierconsultationgynecologique();
        bool bln = false;
        ...
        public void New()
        {
            consultation = new clsdossierconsultationgynecologique();
            bln = false;
        ...
        public void refresh()
        {
            try
            {
                bsrc.DataSource = clsMetier.GetInstance().getAllClsdossierconsultationgynecologique2(clsDoTraitement.idMaladeDossierPre, "Non cloturé non payé", "Payé non cloturé");
            ...
            try
            {
                if (((clsmalade)clsMetier.GetInstance().getClsmalade(consultation.Id_malade)).Sexe == "F")
                {
                    ...
                    else
                    {
                        ...
                        {
                            clsdossierconsultationgynecologique c = (clsdossierconsultationgynecologique)bsrc.Current;
                            new clsdossierconsultationgynecologique().update(c);
                ...
                {
                    MessageBox.Show("Impossible d'enregisrer un homme pour une consultation gynécologique", "Enregistrement", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
            }
            catch (Exception ex)
            {
        ...
        private void FrmDossierConsultationGynecoPaiement_Load(object sender, EventArgs e)
        {
        ...
        private void lstDossierEncCours_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (lstDossierEncCours.Items.Count > 0)
                {
                    ...
                    try
                    {
                        txtPrix.Text = clsMetier.GetInstance().getClstarifconsultationgynecologique(((clstarifconsultationgynecologique)cboService.SelectedItem).Id).Montant.ToString();
                    }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    if (bsrc.DataSource != null)
                    {
                        clsdossierconsultationgynecologique a = (clsdossierconsultationgynecologique)bsrc.Current;
                        new clsdossierconsultationgynecologique().delete(a);
        ...
        {
            if (clsDoTraitement.MAJ_consultationGyn)
            {
                try
                {
                    cboService.DataSource = clsMetier.GetInstance().getAllClstarifconsultationgynecologique();
