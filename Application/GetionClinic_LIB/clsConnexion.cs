...
{ private string server=@"localhost", db, user="sa", pwd="sa", version = "complete";
...
{
   get {
   clsMetier.GetInstance().Initialize(new clsConnexion(server, "Master", user, pwd, version), 1);
 ...
 get { return server; }
  ...
  public string Version
  {
      ...
      set
      {
          ...
          {
              version = value;
              RaisePropertyChanged("Version", value);
          }
      }
  }
 public clsConnexion()
...
public override string ToString()
{
 ...
 public clsConnexion(string server, string db, string user, string pwd,string version)
{
 _errorInfos = new Dictionary<string,string>();
this.server=server;
this.db=db; this.user= user; this.pwd= pwd; this.db=db; this.Version = version;
}
