...
{
    public partial class DossierConsultationPaiementFrm2 : UserControl
    {
        public DossierConsultationPaiementFrm2()
        {
            InitializeComponent();
        }
        ...
        public void refresh()
        {
            try
            {
                bsrc.DataSource = clsMetier.GetInstance().getAllClsconsultation2(clsDoTraitement.idMaladeDossierPre, "Non cloturé non payé", "Payé non cloturé");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        clsconsultation c = (clsconsultation)bsrc.Current;
        ...
        private void lstDossierEncCours_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    txtPrix.Text = clsMetier.GetInstance().getClstarifconsultation(((clstarifconsultation)cboService.SelectedItem).Id).Montant.ToString();
                }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    if (bsrc.DataSource != null)
                    {
                        clsconsultation a = (clsconsultation)bsrc.Current;
                        new clsconsultation().delete(a);
        ...
        {
            if (clsDoTraitement.MAJ_consultation)
            {
                try
                {
                    cboService.DataSource = clsMetier.GetInstance().getAllClstarifconsultation();
