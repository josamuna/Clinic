...
{
    public partial class ConsultationMedecinFrm : Form
    {
        public ConsultationMedecinFrm()
        {
            InitializeComponent();
        }
        ...
        //Consultation
        clsconsultation consultation = new clsconsultation();
        ...
        clsmouvementconsultation mvtconsultation = new clsmouvementconsultation();
        private BindingSource bsrc = new BindingSource();
        private bool bln = false;

        //mouvement maladie
        clsmouvementmaladie mvtmaladie = new clsmouvementmaladie();
        ...
        private void FrmConsultationMedecin_Load(object sender, EventArgs e)
        {
            clsDoTraitement.reinitialiseValue();
            try
            {
                cboMaladie.DataSource = clsMetier.GetInstance().getAllClsmaladie();
        ...
        public void New()
        {
            consultation = new clsconsultation();
            bln1 = false;
        ...
        public void refresh()
        {
            try
            {
                bsrc1.DataSource = clsMetier.GetInstance().getAllClsconsultation2(malade.Id, "Non cloturé non payé", "Payé non cloturé");
                ...
                cboService.DataSource = clsMetier.GetInstance().getAllClstarifconsultation();
        ...
        {
            consultation.Id = ((clsconsultation)lstDossierEncCours.SelectedItem).Id;
        ...
        private void btnSaveDossier_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        clsconsultation c = (clsconsultation)bsrc1.Current;
        ...
        {
            if (ctrl is TextBox) ctrl.DataBindings.Add("Text", mvtconsultation, strValue, true, DataSourceUpdateMode.OnPropertyChanged);
            else if (ctrl is DateTimePicker) ctrl.DataBindings.Add("Text", mvtconsultation, strValue, true, DataSourceUpdateMode.OnPropertyChanged);
            else if (ctrl is ComboBox && valueForCombo == 0) ctrl.DataBindings.Add("Text", mvtconsultation, strValue, true, DataSourceUpdateMode.OnPropertyChanged);
            else if (ctrl is ComboBox && valueForCombo == 1) ctrl.DataBindings.Add("SelectedValue", mvtconsultation, strValue, true, DataSourceUpdateMode.OnPropertyChanged);
        }
        ...
        private void bindingclsmvtConsultation()
        {
            ...
            Control[] tbcontrols = { txtPlaintes, txtSymptome, txtDiagnostics, txtMedicametPrescrit, txtDate, txrIdDossier, txtConduite };
            clearforbindingMvtConsultation(tbcontrols);

            setbindingclsmvtConsultation(txtPlaintes, "Plainte", 0);
            setbindingclsmvtConsultation(txtSymptome, "Symptome", 0);
            setbindingclsmvtConsultation(txtDiagnostics, "Diagnostics", 0);
            setbindingclsmvtConsultation(txtMedicametPrescrit, "Medicamentaprescrire", 0);
            setbindingclsmvtConsultation(txtDate, "Date", 0);
            setbindingclsmvtConsultation(txrIdDossier, "Id_consultation", 0);
            setbindingclsmvtConsultation(txtConduite, "Conduite", 0);
        ...
        private void refreshmvtConsultation()
        {
            try
            {
                bsrc.DataSource = clsMetier.GetInstance().getAllClsmouvementconsultation2(consultation.Id);
                ...
                if (dgvConsultation.SelectedRows.Count > 0)
                {
                    ...
                    bln = true;
                    mvtconsultation.Id = ((clsmouvementconsultation)dgvConsultation.SelectedRows[0].DataBoundItem).Id;
        ...
        private void bindignlsthmvtConsultation()
        {
            ...
            setbindinglstmvtConsultation(txtPlaintes, "Plainte", 0);
            setbindinglstmvtConsultation(txtSymptome, "Symptome", 0);
            setbindinglstmvtConsultation(txtDiagnostics, "Diagnostics", 0);
            setbindinglstmvtConsultation(txtMedicametPrescrit, "Medicamentaprescrire", 0);
            setbindinglstmvtConsultation(txtDate, "Date", 0);
            setbindinglstmvtConsultation(txrIdDossier, "Id_consultation", 0);
            setbindinglstmvtConsultation(txtConduite, "Conduite", 0);

        }

        private void NewmvtConsultation()
        {
            mvtconsultation = new clsmouvementconsultation();
            bln = false;
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                NewmvtConsultation();
                //txrIdDossier.Text
                txrIdDossier.Text = consultation.Id.ToString();
        ...
        private void dgvConsultation_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (dgvConsultation.SelectedRows.Count > 0)
                {
                    ...
                    //Chargement des résultats de l'examen
                ...
                else
                {
                    ...
                    else
                    {
                        if (bsrc.DataSource != null)
                        {
                            ...
                            new clsmouvementconsultation().update(m);
                            MessageBox.Show("Modification effectuée!", "Modification", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                }
        ...
        {
            MaladieFrm form = new MaladieFrm();
        ...
        {
            if (clsDoTraitement.EnterFormMaladie)
            {
                try
                {
                    cboMaladie.DataSource = clsMetier.GetInstance().getAllClsmaladie();
        ...
        private void btnAddMouvementMaladie_Click(object sender, EventArgs e)
        {
            ...
            {
                NewMvtMaladie();
                txtIdMouvementCons.Text = mvtconsultation.Id.ToString();
            }
            catch (Exception)
            {
                btnSaveDossier.Enabled = false;
        ...
        {
            mvtmaladie = new clsmouvementmaladie();
            bln2 = false;
        ...
        private void bindingclsMouvementMaladie()
        {
            ...
            setbindingclsMouvementMaladie(txtIdMouvementCons, "Id_mouvementconsultation", 0);
        }
        ...
        private void bindignlstMouvementMaladie()
        {
            ...
            setbindinglstMouvementMaladie(txtIdMouvementCons, "Id_mouvementconsultation", 0);
        }
        ...
        public void refreshMouvementMaladie()
        {
            try
            {
                bsrc2.DataSource = clsMetier.GetInstance().getAllClsmouvementmaladie2(mvtconsultation.Id);
                dgvMedoc.DataSource = bsrc2;
        ...
        private void btnSaveMouvementMaladie_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        clsmouvementmaladie m = (clsmouvementmaladie)bsrc2.Current;
                        new clsmouvementmaladie().update(m);
                        MessageBox.Show("Modification effectuée!", "Modification", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
        ...
        private void btnDeleteMouvementMaladie_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        clsmouvementmaladie m = (clsmouvementmaladie)bsrc2.Current;
                        new clsmouvementmaladie().delete(m);
                        MessageBox.Show("Suppression éffectuée", "Suppression", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.NewMvtMaladie();
                        refreshMouvementMaladie();
                    }
        ...
        private void cboDateExamen_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    dataGridView1.DataSource = clsMetier.GetInstance().getClsResultatExamen_mouvementconsultationt(malade.Id, cboDateExamen.Text.Substring(0, 10));
                    foreach (DataGridViewColumn dgvc in dataGridView1.Columns)
                    {
                        if (dgvc.DataPropertyName == "Id") dgvc.Visible = false;
                        else if (dgvc.DataPropertyName == "Id_critere") dgvc.Visible = false;
                        else if (dgvc.DataPropertyName == "Id_operation_laboratoire") dgvc.Visible = false;
                        else if (dgvc.DataPropertyName == "Date") dgvc.Visible = false;
                    }
                }
                else
                {
        ...
        private void tabControl1_Selecting(object sender, TabControlCancelEventArgs e)
        {
            if (tabControl1.SelectedIndex == 2)
            {
                try
                {
                    cboAllergie.DataSource = clsMetier.GetInstance().getAllClsallergie();
                    setMembersallcbo(cboAllergie, "Designation", "Id");
                    cboMaladieA.DataSource = clsMetier.GetInstance().getAllClsmaladie();
        ...
        private void cmdLoadData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    //bsrcl.DataSource = clsMetier.GetInstance().getAllClsmouvementoperation_laboratoire3(malade.Id);
                    //cboDateExamen.DataSource = bsrcl;
